...
initial_state, final_states, string, transition_function, arr = csv_dict_reader('first_machine_structure.csv')
s = []
for i in string:
...
print('Результат хранится в файле rock_1.csv')
file = 'rock_1'
file_print(arr, file)
print()

print('---Вторая Машина: прибавляет единицу к каждому символу в двоичном представлении---')
initial_state, final_states, string, transition_function, arr = csv_dict_reader('second_machine_structure.csv')
s = []
for i in string:
    current_state = initial_state
    head_position = 0
...
for i in range(len(s)):
    arr[0][i + 2] = "".join(s[i])[:len(s[i]) - 1]
print('Результат хранится в файле rock_2.csv')
file = 'rock_2'
file_print(arr, file)
